 ...
 *<PRE>
 * 入力データ読み込み。
 * ver0.1 2014/05/02 新規作成
 *</PRE>
 * @param pccFileName    ファイル名
 * @param pccMode        ファイル読み込み read ファイル書き込み write
 * @throw CCsvException
 * @author kobayashi
 * @since 2014/05/02
 * @version 0.1
 */
void CCsv::Open( const char* pccFileName, const char *pccMode )
{
	CCsvException csve;
	...
	try
	{
		...
		else if( pccMode == "write" )
		{
			fp = fopen( pccFileName, "w+b" );
			if( fp == NULL )
			{
				csve.SetErrorInfo( CSV_FILE_OPEN_ERROR, "Open", "CCsv", "ファイル読み込みでエラー発生", __LINE__ );
				throw( csve );
 ...
 *<PRE>
 * 入力データ読み込み。
 * ver0.1 2014/05/05 新規作成
 *</PRE>
 * @param ppvData  読み込むデータ
 * @param iColumn  読み込む行数
 * @param iRow     読み込む列数
 * @param srtType  読み込むデータの型
 * @throw CCsvException
 * @author kobayashi
...
void CCsv::Read( void **pvData, int iColumn, int iRow, std::string strType )
{
	...
	try
	{
		...
		/* 列の読み込み */
			...
			{
				//データの読み込みをする。
				_DataRead( pvData, iCurColumn, iCurRow, acBuf, strType );
 ...
 *<PRE>
 * 入力データ読み込み。
 * ver0.1 2014/05/10 新規作成
 *</PRE>
 * @param pppvData  読み込むデータ
 * @param iColumn  読み込む行数
 * @param iRow     読み込む列数
 * @param srtType  読み込むデータの型
 * @throw CCsvException
 ...
 * @version 0.1
 */
void CCsv::Read( void ***pppvData, int iColumn, int iRow, std::string strType )
{
	int iBufCount = 0;
	int iDataNum = 0;
	int iCurRow = 0;
	...
	try
	{
		...
		/* 列の読み込み */
			...
			{
				//データの読み込みをする。
				_DataRead( pppvData, iCurColumn, iCurRow, acBuf, strType );
...
void CCsv::_DataRead( void *pvData, int iDataNum, const char *pccBuf, std::string strType )
{
	...
	{
		*((int*)pvData+iDataNum) = (int)atoi( pccBuf );
	}
	else if( strType == "short" )
	{
		*((short*)pvData+iDataNum) = (short)atoi( pccBuf );
	}
	else if( strType == "long" )
	{
		*((long*)pvData+iDataNum) = (long)atol( pccBuf );
	}
	else if( strType == "char" )
	{
 ...
 *</PRE>
 * @param ppvData     読み込むデータ
 * @param iColumn     読み込む現在の行数
 * @param iRow        読み込む現在の列数
 * @param srtType     読み込むデータの型
 * @author kobayashi
 * @since 2014/05/05
 * @version 0.1
 */
void CCsv::_DataRead( void **ppvData, int iColumn, int iRow, const char *pccBuf, std::string strType )
{
	//データの読み込みをする。
	if( strType == "double" )
	{
		((double**)ppvData)[iRow][iColumn] = atof( pccBuf );
	}
	else if( strType == "float" )
	{
		((float**)ppvData)[iRow][iColumn] = atof( pccBuf );
	}
	else if( strType == "int" )
	{
		((int**)ppvData)[iRow][iColumn] = (int)atoi( pccBuf );
	}
	else if( strType == "short" )
	{
		((short**)ppvData)[iRow][iColumn] = (short)atoi( pccBuf );
	}
	else if( strType == "long" )
	{
		((long**)ppvData)[iRow][iColumn] = (long)atol( pccBuf );
	}
	else if( strType == "char" )
	{
		strcpy( ((char**)ppvData)[iRow], pccBuf );
	}
	else
	{
		((double**)ppvData)[iRow][iColumn] = atof( pccBuf );
 ...
 * @version 0.1
 */
void CCsv::_DataRead( void ***pppvData, int iColumn, int iRow, const char *pccBuf, std::string strType )
{
	//データの読み込みをする。
	if( strType == "char" )
	{
		strcpy( ((char***)pppvData)[iRow][iColumn], pccBuf );
	}
	else
	{
//		((double**)ppvData)[iRow][iColumn] = atof( pccBuf );
